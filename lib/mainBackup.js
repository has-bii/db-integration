...
const mainBackup = async (configuration) => {
    ...
    try {
      ...
      for (const table of conf.tables) {
        try {
          ...
          console.error("Error in Table Section");

          writeErrorLog(error);

          if (error.tables) tableErrorConfig(error.tables);
        }
      }
      ...
      if (error.conf) writeErrorConfig(error.conf);
    }
  }
};
