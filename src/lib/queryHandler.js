...
const getDataByTimestamp = async (lastData, source_db, table) => {
  // increment millisecond by 1
  const timestamp = new Date(lastData[table.filterByCol.target])

  const millisecond = timestamp.getMilliseconds()

  const newTimestamp = new Date(timestamp).setMilliseconds(millisecond + 1)
  ...
  const data = await source_db.query(
    `SELECT * FROM ${table.sourceTable} WHERE ${table.filterByCol.source} >= :filteredData`,
    {
      type: QueryTypes.SELECT,
      replacements: {
        filteredData: new Date(newTimestamp),
      },
    }
  )
...
function dataToQuery(target_db, data, columns) {
  const query = data
    .map((value) => {
      const cols = columns.map((col) => target_db.escape(value[col])).join(", ")
